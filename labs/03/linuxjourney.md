# Quizantworten (lab 03)

1. Quizantwort bei der Sektion The Shell:  
What is the exact output to the display when you type `echo Hello World`? Please answer in English, paying close attention to capitalization and spacing.
```bash
echo Hello World
```

2. Quizantwort bei der Sektion pwd (Print Working Directory):  
Which command is used to find the directory you are currently in? (Please answer in English, using only the command name in lowercase.)
```bash
pwd
```

3. Quizantwort bei der Sektion cd (Change Directory):  
If you are in `/home/pete/Pictures` and want to navigate to the parent directory (`/home/pete`), what is the full command you should use? Please answer in English, paying attention to case and spacing.
```bash
cd ..
```

4. Quizantwort bei der Sektion ls (List Directories):  
What command would you use to see hidden files? Please answer in English, paying attention to case sensitivity.
```bash
ls -a
```

5. Quizantwort bei der Sektion touch:  
How do you create a file called `myfile`? Please answer using only the English command, paying attention to case sensitivity.
```bash
touch myfile
```

6. Quizantwort bei der Sektion file: 
What command can you use to find the file type of a file?
```bash
file
```

7. Quizantwort bei der Sektion cat:  
What command is used to display the contents of a file on the command line? (Note: Your answer should be a single, lowercase English word.)
```bash
cat
```

8. Quizantwort bei der Sektion less:  
How do you quit out of the `less` command? Please provide the single character key as your answer. Note: the answer is a case-sensitive English letter.
```bash
q
```

9. Quizantwort bei der Sektion history:  
What is the command to clear the terminal? (Please answer in lowercase English letters only)
```bash
clear
```

10. Quizantwort bei der Sektion cp (Copy):  
What flag do you need to specify to copy over a directory?
```bash
-r
```

11. Quizantwort bei der Sektion mv (Move):  
Using the `mv` command, how would you rename a file named `cat` to `dog`? Please provide the full command. Note: The answer is case-sensitive and should be entered in lowercase English.
```bash
mv cat dog
```

12. Quizantwort bei der Sektion mkdir (Make Directory):  
What command is used to make a directory? Please answer using only the lowercase English command.
```bash
mkdir
```

13. Quizantwort bei der Sektion rm (Remove):  
How do you remove a file named `myfile`? Your answer must be in English and use the exact, case-sensitive command.
```bash
rm myfile
```

14. Quizantwort bei der Sektion find:  
What option should you specify for the `find` command to search by name? Please answer using only the English option, paying attention to the required format (e.g., -option).
```bash
- name
```

15. Quizantwort bei der Sektion help:  
How do you get quick command-line help for built-in Bash commands? (Please provide the single command name in English and in lowercase.)
```bash
help
``` 

16. Quizantwort bei der Sektion man:  
How do you see the manual for a command? (Please answer using only the command name in lowercase English letters).
```bash
man
``` 

17. Quizantwort bei der Sektion whatis:  
What command can you use to see a small description of a command? Please answer in English, paying attention to lowercase.
```bash
whatis
```  

18. Quizantwort bei der Sektion alias:  
What command is used to create an alias? Please answer in lowercase English.
```bash
alias
```

19. Quizantwort bei der Sektion exit:  
What is the most common command to exit from the Linux shell? Please answer using only a single lowercase English word.
```bash
exit
```
